...
const AutocompleteTextarea: React.FC<AutocompleteTextareaProps> = ({
  ...
  // Handle textarea blur with delay to allow for suggestion clicks
  const handleBlur = () => {
    setTimeout(() => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    switch (e.key) {
      case 'ArrowDown':
        // Only handle if Ctrl/Cmd is pressed to avoid interfering with normal textarea navigation
      ...
      case 'Enter':
        if ((e.ctrlKey || e.metaKey) && selectedIndex >= 0 && selectedIndex < suggestions.length) {
          e.preventDefault();
      ...
      {showSuggestions && suggestions.length > 0 && (
        ...
        >
          <div className="p-2 bg-gray-50 border-b border-gray-200 text-xs text-gray-600">
            Use Ctrl/Cmd + Arrow keys to navigate, Ctrl/Cmd + Enter to select
          </div>
